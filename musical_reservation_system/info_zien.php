...
<div>
<?php
echo '<table style="width:100%; text-align:center;background-color:#2f82c7;">
<tr>
	<td style="border-style: solid;width=33.33%;"><a href="index.php" style="color:White;font-family:Arial;font-size: 25px;">Home</a></td><td style="border-style: solid;width=33.33%;"><a href="reserveren.php" style="color:White;font-family:Arial;font-size: 25px;">Reserveren</a></td>';
if ($rol == 0){
	echo '<td style="border-style: solid;width=33.33%;"><a href = "info_zien.php" style="color:White;font-family:Arial;font-size: 25px;">Inloggen</a></td>';
}else{
	echo '<td style="border-style: solid;width=33.33%;"><a href = "uitloggen.php" style="color:White;font-family:Arial;font-size: 25px;">Uitloggen</a></td>';
}
echo '</tr> </table></div><div style="width:95%;background-color:#2f82c7; position: absolute; top: 200px;left: 16px; padding:20px">';
if ($rol == 0){
	echo '<h1 style="color:White;font-family:Arial;font-size: 50px;text-align:center;">Inloggen</h1>';
	echo "<p style='font-family:Arial;font-size: 25px;'>Voer uw gebruikersnaam en wachtwoord in om door te gaan.</p>
	<form name='inloggen' method='post' action='#' >					
		<p style='font-family:Arial;font-size: 25px;'>Gebruikersnaam: <input type='textfield' name='username' ></p>
		<p style='font-family:Arial;font-size: 25px;'>Wachtwoord:<input type='textfield' name='password'></p>
		<input style='width:200px;hight:100px;font-family:Arial;font-size: 25px;'type='submit' value='Inloggen' name='inloggen_2'><br><br>
	</form>";
}
	   ...
	   // output data of each row
		while($row = $result->fetch_assoc()) {
			if ($username == $row["username"] && $password == $row["password"]){
				...
				header("Refresh:0; url=info_zien.php");
				break;
			}
		}
		echo "Gebruikersnaam en/of wachtwoord klopt niet";
	}else {
		echo "0 results <br>";
	}
}
if ($rol == 1){
	...
	if (isset($_POST['reserveringen']) or isset($_POST['bevestigen']) or isset($_POST['verwijder_gegevens'])or isset($_POST['aanpassen']) or isset($_POST['all']) or isset($_POST['woensdag_avond']) or isset($_POST['donderdag_avond']) or isset($_POST['vrijdag_avond']) or isset($_POST['vrijdag_middag'])){
		$all = true;
		echo "<h1 style='color:White;font-family:Arial;font-size: 50px;text-align:center;'> Reserveringen </h1>";
		echo '<form method="post" action='.htmlspecialchars($_SERVER["PHP_SELF"]).'>';
	...
	if (isset($_POST['zaal_woensdag_avond'])){
		echo "<h1 style='color:White;font-family:Arial;font-size: 50px;text-align:center;'> Woensdag avond </h1>";
	...
	if (isset($_POST['zaal_donderdag_avond'])){
		echo "<h1 style='color:White;font-family:Arial;font-size: 50px;text-align:center;'>Donderdag avond </h1>";
	...
	if (isset($_POST['zaal_vrijdag_middag'])){
		echo "<h1 style='color:White;font-family:Arial;font-size: 50px;text-align:center;'> Vrijdag middag </h1>";
	...
	if (isset($_POST['zaal_vrijdag_avond'])){
		echo "<h1 style='color:White;font-family:Arial;font-size: 50px;text-align:center;'> Vrijdag avond </h1>";
